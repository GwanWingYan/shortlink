            ...
            <ul class="sortOptions">
                <li v-for="(item, index) in editableTabs" :key="item.gid">
                    <div class="item-box flex-box hover-box" :class="{ selectedItem: selectedIndex === index }"
                        ...
                        <div class="flex-box">
                            <!-- 图标 -->
                            <el-tooltip show-after="500" class="box-item" effect="dark" :content="'查看图表'"
                                placement="bottom-end">
                                <!-- 传group是为了表示这个请求是查询分组图表数据 -->
                                <el-icon v-if="!(item.shortlinkCount === 0 || item.shortlinkCount === null)"
                                    class="edit" :class="{ zero: item.shortlinkCount === 0 }"
                            ...
                            <el-dropdown>
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu>
                                        <el-dropdown-item
                                            @click="showEditGroup(item.gid, item.name)">编辑</el-dropdown-item>
            ...
            <div class="table-box">
                ...
                <div v-if="!isRecycleBin" class="buttons-box">
                    <div style="width: 100%; display: flex">
                        ...
                        <el-button style="width: 130px; margin-right: 10px"
                            @click="isAddSmallLinks = true">批量创建</el-button>
                    </div>
                </div>
                <!-- 展示回收站信息 -->
                    ...
                    <el-table-column label="短链接信息" prop="info" min-width="300">
                        <template #header>
                            <el-dropdown>
                                ...
                                <template #dropdown>
                                    <el-dropdown-item
                                        @click="pageParams.orderTag = null, orderIndex = 0">创建时间</el-dropdown-item>
                                </template>
                            </el-dropdown>
                        </template>
                        <template #default="scope">
                            <div class="table-link-box" :class="{
                                ...
                                <img :src="getImgUrl(scope.row.favicon)" :key="scope?.row?.id" width="20" height="20"
                                    alt="" />
                                <div class="name-date">
                                    ...
                                    <div class="time" style="display: flex">
                                        ...
                                        <el-tooltip show-after="500" v-if="scope?.row?.validDate"
                                            ...
                                            <img v-if="isExpire(scope?.row?.validDate)" width="18" height="18"
                                                src="@/assets/png/沙漏倒计时.png" alt="" />
                    ...
                    <el-table-column label="访问次数" prop="times" width="120">
                        <template #header>
                            <el-dropdown>
                                ...
                                <template #dropdown>
                                    <el-dropdown-item
                                        @click="pageParams.orderTag = 'todayPv', orderIndex = 1">今日访问次数</el-dropdown-item>
                                    <el-dropdown-item
                                        @click="pageParams.orderTag = 'totalPv', orderIndex = 1">累计访问次数</el-dropdown-item>
                                </template>
                            </el-dropdown>
                        </template>
                        <template #default="scope">
                    ...
                    <el-table-column label="访问人数" prop="people" width="120">
                        <template #header>
                            <el-dropdown>
                                ...
                                <template #dropdown>
                                    <el-dropdown-item
                                        @click="pageParams.orderTag = 'todayUv', orderIndex = 2">今日访问人数</el-dropdown-item>
                                    <el-dropdown-item
                                        @click="pageParams.orderTag = 'totalUv', orderIndex = 2">累计访问人数</el-dropdown-item>
                                </template>
                            </el-dropdown>
                        </template>
                        <template #default="scope">
                    ...
                    <el-table-column label="IP数" prop="IP" width="120">
                        <template #header>
                            <el-dropdown>
                                ...
                                <template #dropdown>
                                    <el-dropdown-item
                                        @click="pageParams.orderTag = 'todayUip', orderIndex = 3">今日IP数</el-dropdown-item>
                                    <el-dropdown-item
                                        @click="pageParams.orderTag = 'totalUip', orderIndex = 3">累计IP数</el-dropdown-item>
                                </template>
                            </el-dropdown>
                        </template>
                        <template #default="scope">
                        ...
                        <template #default="scope">
                            <div style="display: flex; align-items: center">
                                ...
                                <template v-if="selectedIndex !== -1">
                                    <!-- 表格中的编辑按钮 -->
                                    <el-tooltip show-after="500" class="box-item" effect="dark" content="编辑"
                                        placement="bottom-end">
                                    ...
                                    <!-- 删除按钮 -->
                                    <el-tooltip show-after="500" class="box-item" effect="dark" content="删除"
                                        placement="bottom-end">
                                ...
                                <template v-else>
                                    <!-- 回收站中的恢复按钮 -->
                                    <el-tooltip show-after="500" class="box-item" effect="dark" content="恢复"
                                        placement="bottom-end">
                ...
                <div class="pagination-block">
                    <el-pagination v-model:current-page="pageParams.current" v-model:page-size="pageParams.size"
                        ...
                        :total="totalNums" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
                </div>
            </div>
        </div>
        <!-- 查看数据弹框 -->
        ...
        <el-dialog @close="afterAddLink" v-model="isAddSmallLink" title="创建链接">
            <el-tabs class="demo-tabs">
                <el-tab-pane>
                    ...
                    <CreateLink ref="createLink1Ref" :groupInfo="editableTabs" @onSubmit="addLink"
                        @cancel="cancelAddLink" :defaultGid="pageParams.gid" :is-single="true"></CreateLink>
                </el-tab-pane>
            </el-tabs>
        </el-dialog>
        <!-- 修改短链信息弹框 -->
...
const chartsVisible = async (rowInfo, dateList) => {
    ...
    const { lid, gid, group, originUrl, favicon, enableStatus } = rowInfo
    ...
    let tableRes = null
    if (group) {
        res = await API.group.queryGroupStats({ ...statsFormData, lid, gid })
        tableRes = await API.group.queryGroupTable({ gid, ...statsFormData })
    } else {
        res = await API.shortlink.queryLinkStats({ ...statsFormData, lid, gid, enableStatus })
        tableRes = await API.shortlink.queryLinkTable({ gid, lid, ...statsFormData, enableStatus })
    }
...
const changeTimeData = async (rowInfo, dateList) => {
    ...
    if (isGroup.value) {
        ...
        res = await API.shortlink.queryLinkStats({ ...statsFormData, lid, gid })
        tableRes = await API.shortlink.queryLinkTable({ gid, lid, ...statsFormData })
    }
    tableInfo.value = tableRes
    chartsInfo.value = res?.data?.data
}
...
const changePage = async (page) => {
    ...
    if (isGroup.value) {
        ...
        tableRes = await API.shortlink.queryLinkTable({
            gid: tableGid.value,
            lid: tablelid.value,
            ...statsFormData
        })
...
const queryPage = async () => {
    ...
    const res = await API.shortlink.queryPage(pageParams)
    if (res?.data.success) {

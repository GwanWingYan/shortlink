    ...
    <div>
        <el-form ref="ruleFormRef" :model="formData" :rules="formRule" label-width="80px">
            <el-form-item label="跳转链接" prop="originUrl">
                <el-input v-if="isSingle" v-model="formData.originUrl"
                    placeholder="请输入http://或https://开头的链接或应用跳转链接"></el-input>
            ...
            <el-form-item label="描述信息" prop="describe">
                <el-input maxlength="100" show-word-limit v-loading="isLoading" :rows="4" v-model="formData.describe"
            ...
            <el-form-item v-if="formData.validDateType === 1" label="选择时间">
                <el-date-picker :disabled-date="disabledDate" v-model="formData.validDate"
                    value-format="YYYY-MM-DD HH:mm:ss" type="datetime" placeholder="选择日期" :shortcuts="shortcuts" />
...
const queryTitle = (url) => {
    if (httpRegex.test(url)) {
        ...
        API.shortlink.queryTitle({ url: url }).then((res) => {
...
const onSubmit = async (formEl) => {
    ...
    await formEl.validate(async (valid, fields) => {
        if (valid) {
            const res = await API.shortlink.addSmallLink(formData)
            if (!res?.data?.success) {

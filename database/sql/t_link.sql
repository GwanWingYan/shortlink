...
(
    ...
    `enable_flag`     tinyint(1)                                     DEFAULT NULL COMMENT '启用标识: 0启用, 1未启用',
    `create_type`     tinyint(1)                                     DEFAULT NULL COMMENT '创建类型: 0接口创建, 1控制台创建',
    ...
    `delete_time`     bigint(20)                                     DEFAULT '0' COMMENT '删除时间戳',
    `delete_flag`     tinyint(1)                                     DEFAULT NULL COMMENT '删除标识: 0未删除, 1已删除',
    UNIQUE KEY `id` (`id`),
    UNIQUE KEY `idx_unique_lid` (`lid`, `delete_time`) USING BTREE
) ENGINE = InnoDB
